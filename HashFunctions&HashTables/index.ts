...
function customHashFunction(key: string, length: number = 8): string {
  let hash = 0;
...
Colisions are inevitable when hashing a wide range of keys in a fixed size array
To do this task I will implement a small hash table to ilustrate a real case collision.

Separate Chaining
In separate chaining, each slot in the hash table holds a list (or another kind of collection) of all entries that hash to the same value.
...
I'm going to conbine collisions with building a hash table to kill two birds in a shot
*/
...
interface IHashTableClass {
  size: number;
  table: Map<string, HashTableEntry[]>;
  get(key: string): any;
  set(key: string, value: any): void; // Fixed typo here
  delete(key: string): void; // Added delete method
  customHashFunction(key: string): string;
}

class HashTableClass implements IHashTableClass {
  size: number;
  table: Map<string, HashTableEntry[]>;
  ...
  public set(key: string, value: any): void {
    ...
    let tableEntry: HashTableEntry[] | undefined = this.table.get(index);
  ...
  public get(key: string): any {
    ...
    let tableEntry: HashTableEntry[] | undefined = this.table.get(index);
    if (tableEntry !== undefined) {
      const hashTableEntry: HashTableEntry | undefined = tableEntry.find(
        entry => entry.key === key
      );
  ...
  public delete(key: string): void {
    ...
    if (tableEntry !== undefined) {
      this.table.set(
        index,
        tableEntry.filter(entry => entry.key !== key)
      );
    } else {
      throw new Error(`The key ${key} doesn't exist in the HASH TABLE`);
    }
...
class HashTableClassArray {
  size: number;
  table: Array<HashTableEntry[] | null>;
  ...
  customHashFunction(key: string): number {
    let hash = 0;
...
class HashTableClassMapResizeHas {
  size: number;
  table: Map<string, HashTableEntry[]>;
  entries: number; // to keep track of the total number of keys
  loadFactor: number;
...
class HashTableClassArrayResizeHas {
  ...
  private customHashFunction(key: string): number {
    let hash = 0;
    for (let i = 0; i < key.length; i++) {
      hash += key.charCodeAt(i);
    }
  ...
  private resize(): void {
    if (this.entries <= this.size * this.loadFactor) return; // if under the load factor, return early

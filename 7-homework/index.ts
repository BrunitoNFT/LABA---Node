...
interface analysis {
  arrLength: number;
  quickSortTime: arrayTypeRes;
  bubbleSortTime: arrayTypeRes;
  mergeSortTime: arrayTypeRes;
  faster: 'bubble' | 'merge' | 'quick' | '-';
}

let results: analysis[] = [];

function testSortAlgorithm(arr: number[], sortAlgorithm: string) {
...
let consecutiveTimes: {
  method: 'bubble' | 'merge' | 'quick' | '-';
  times: number;
  arrLength: number;
} = { method: '-', times: 0, arrLength: 0 };
...
for (let index = 0; index < 40; index++) {
  ...
  let sortFastest =
    quickTimeSorted < bubbleTimeSorted && quickTimeSorted < mergeTimeSorted
      ? 'quick'
  ...
  let reverseFastest =
    quickTimeReverse < bubbleTimeReverse && quickTimeReverse < mergeTimeReverse
      ? 'quick'
  ...
  let randomFastest =
    quickTimeRandom < bubbleTimeRandom && quickTimeRandom < mergeTimeRandom
      ? 'quick'
  ...
  consecutiveTimes = {
    ...
    method: winner,
    arrLength:
      consecutiveTimes.method === winner
...
console.log(
  'Array Length | QuickSort Time (ms)     | BubbleSort Time (ms)      | MergeSort Time (ms)        | Faster'
);
console.log('-'.repeat(90));

for (const result of results) {
  console.log(
    `${result.arrLength.toString().padEnd(13)} ` +
      `${(
        'Ra: ' +

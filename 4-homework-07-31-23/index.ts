...
function deleteNonConfigurable(obj: any, propertyName: any) {
...
const bankAccount = {
  ...
  transfer(targetAccount: any, amount: number) {
    if (typeof amount !== 'number' || amount < 0) {
      throw new Error('Amount should be a positive number.');
    }
...
function createImmutableObject(obj: any, clones = new WeakMap()): any {
...
function observeObject(
  obj: any,
  callback: (property: string, action: string) => void
) {
  if (typeof obj !== 'object' || obj === null) {
...
function deepCloneObject(obj: any, clones = new WeakMap()): any {
  if (typeof obj !== 'object' || obj === null) {
    return obj;
  }
...
interface DynamicValidationSchema {
  [key: string]: ValidationFunction;
}

function validateObject(obj: any, schema: DynamicValidationSchema): boolean {
  if (typeof obj !== 'object' || obj === null) {
    return false;
  }

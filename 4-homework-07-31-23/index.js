...
person.updateInfo = function (newInfo) {
    Object.keys(newInfo).forEach(propertyName => {
        if (!person.hasOwnProperty(propertyName)) {
            console.log(`Property ${propertyName} does not exist in the object. Skipping...`);
            return;
        }
        const propertyDescriptor = Object.getOwnPropertyDescriptor(person, propertyName);
        if (!propertyDescriptor) {
            console.log(`Property ${propertyName} does not have a descriptor. Skipping...`);
            return;
        }
        if (!propertyDescriptor.configurable || !propertyDescriptor.writable) {
            console.log(`Cannot update non-configurable or non-writable property: ${propertyName}. Skipping...`);
            return;
        }

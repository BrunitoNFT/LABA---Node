...
type CustomFilterCallback<T> = (value: T, index: number, array: T[]) => boolean;

function customFilterUnique<T>(
  array: T[],
...
function chunkArray<T>(arr: T[], chunkSize: number): T[][] {
  ...
  while (index < arr.length) {
    const chunk: T[] = new Array(Math.min(chunkSize, arr.length - index));
...
function getArrayUnion<T>(array1: T[], array2: T[]): T[] {
...
function measureArrayPerformance<T, U>(
  func: (arr: T[], callback: (value: T, index: number, array: T[]) => U) => any,
  arr: T[],

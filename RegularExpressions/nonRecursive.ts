  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (token.match(/^\{$/)) {
      ...
      if (
        ...
        if (tokens[index + 1] === ':') {
          ...
          throw new Error(
            'Syntax error. The next item of a key is not a colon.'
          );
        }
    ...
    if (stackPosition.length > 0) {
      if (Array.isArray(stackPosition[stackPosition.length - 1])) {
        ...
        if (currentKey) {
          stackPosition[stackPosition.length - 1][currentKey] = parserData(
            token
          );
          currentKey = null;
          continue;
          ...
          throw new Error(
            "The current key doesen't exist: " +
              currentKey +
              ' And the token is: ' +
              token
          );
...
function parserData(token: string): any {
  if (token.match(/^(-?\d+)$/)) {
